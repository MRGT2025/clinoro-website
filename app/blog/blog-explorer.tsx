...
export function BlogExplorer({posts}:{posts:BlogPost[]}){
  ...
  return <>
    ...
    <div className="blog-grid">{posts.map((post,index)=><article className={`blog-card prism-edge${index===0?" featured":""}`} key={post.id}>
      <Link className="blog-card-image" href={`/blog/${post.slug}`}><Image src={post.image||"/assets/medical-visual.jpg"} alt={post.title} fill unoptimized priority={index<2} sizes="(max-width:800px) 100vw,50vw"/></Link>
