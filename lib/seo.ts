...
export async function pageMetadata(key:Exclude<PageKey,"home">):Promise<Metadata>{
  ...
  const english=key==="products"?"/en/products":key==="contact"?"/en/contact":undefined;
  return {title:page.title,description:page.text,alternates:{canonical:url,...(english?{languages:{"fa-IR":url,en:english}}:{})},openGraph:{title:`${page.title} | Clinoro`,description:page.text,url,images:[{url:page.image,alt:page.title}]},twitter:{card:"summary_large_image",title:`${page.title} | Clinoro`,description:page.text,images:[page.image]}};
}
