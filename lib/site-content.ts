...
function mergeContent(base:SiteContent,value:Partial<SiteContent>):SiteContent{
  ...
  const mergedPosts=(value.schemaVersion??0)<7
  ...
  return {
    ...
    blogPosts:mergedPosts.map(post=>({...post,sources:Array.isArray(post.sources)?post.sources:[],imageCredit:post.imageCredit||"",imageSource:post.imageSource||""})),
